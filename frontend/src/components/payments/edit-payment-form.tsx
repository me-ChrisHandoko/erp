 ...
 *
 * Professional form for editing existing supplier payments with:
 * - Pre-filled data from existing payment
...
import { useUpdatePaymentMutation } from "@/store/services/paymentApi";
...
export function EditPaymentForm({
  ...
  const handleChange = (field: keyof UpdatePaymentRequest, value: any) => {
    setFormData((prev) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle specific error for approved payments
      if (error?.data?.error?.includes("approved")) {
        toast.error("Pembayaran yang sudah disetujui tidak dapat diedit");
      } else {
        toast.error(
          error?.data?.error || "Gagal memperbarui pembayaran. Silakan coba lagi."
        );
      }
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Alert className="bg-blue-50 border-blue-200 dark:bg-blue-900/10 dark:border-blue-900">
        ...
        <AlertDescription className="text-blue-800 dark:text-blue-200">
          Perubahan pada pembayaran akan mempengaruhi catatan keuangan. Pastikan data yang Anda masukkan sudah benar.
        </AlertDescription>
      </Alert>
