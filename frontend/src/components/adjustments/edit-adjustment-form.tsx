/**
 * Edit Adjustment Form Component
 *
 * Pre-filled form for editing DRAFT adjustments with same structure as create form
 */
...
import { useUpdateAdjustmentMutation } from "@/store/services/adjustmentApi";
...
export function EditAdjustmentForm({
  ...
  // Pre-fill form with adjustment data
  useEffect(() => {
    if (adjustment) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setErrors({
        submit: error?.data?.message || "Gagal memperbarui penyesuaian",
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {currentStep === 1 && (
        <Card>
          ...
          <CardContent className="space-y-4">
            {/* Same content as CreateAdjustmentForm Step 1 */}
      ...
      {/* Step 2 & 3: Same as CreateAdjustmentForm (product selection and review) */}
      {/* Abbreviated for brevity - same structure as create form */}
      ...
      <div className="flex items-center justify-between gap-4">
        <Button
          type="button"
          variant="outline"
          onClick={onCancel || (() => router.push(`/inventory/adjustments/${adjustment.id}`))}
          disabled={isUpdating}
        >
          Batal
        </Button>
