 ...
 *
 * Professional form for creating new customers with:
 * - Real-time validation
 * - Currency formatting for credit limit
 * - Multi-section layout (Basic Info, Contact, Address, Business Terms)
 * - Responsive design
 */
...
import { useCreateCustomerMutation } from "@/store/services/customerApi";
...
export function CreateCustomerForm({
  ...
  const validate = (): boolean => {
    ...
    if (!formData.code.trim()) {
      newErrors.code = "Kode pelanggan wajib diisi";
    ...
    if (!formData.customerType) {
      newErrors.customerType = "Tipe pelanggan wajib dipilih";
    }

    // Phone validation (Indonesian format - optional but validate if provided)
    ...
    const creditTermDays = Number(formData.creditTermDays || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await createCustomer(cleanedData).unwrap();
