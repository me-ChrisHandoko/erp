 ...
 *
 * Multi-step wizard form for creating stock transfers:
...
export function CreateTransferForm({
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = await createTransfer(requestData).unwrap();

      toast.success("✓ Transfer Berhasil Dibuat", {
        description: `Transfer ${result.transferNumber} telah dibuat dan siap untuk diproses.`,
      });

      // Navigate to detail page or call onSuccess
      if (onSuccess) {
        onSuccess(result.id);
      } else {
        router.push(`/inventory/transfers/${result.id}`);
      }
    } catch (error: any) {
      console.error("❌ [CreateTransfer] Error:", error);

      toast.error("Gagal Membuat Transfer", {
        description: error?.data?.message || "Terjadi kesalahan saat membuat transfer. Silakan coba lagi.",
      });

      setErrors({
        submit: error?.data?.message || "Gagal membuat transfer",
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <Button
          type="button"
          variant="outline"
          onClick={onCancel || (() => router.push("/inventory/transfers"))}
          disabled={isCreating}
        >
