 ...
 *
 * Server-side rendered page that fetches initial purchase order data
...
import type { PurchaseOrderListResponse } from "@/types/purchase-order.types";
...
export default async function PurchaseOrdersPage() {
  ...
  // Fetch initial purchase orders data on server
  let initialData: PurchaseOrderListResponse;

  try {
    const response = await apiFetch<ApiSuccessResponse<PurchaseOrderListResponse>>({
    ...
    console.error('[Purchase Orders Page] Failed to fetch initial data:', error);
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-1 flex-col items-center justify-center min-h-[400px] gap-4 p-4">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {error instanceof Error ? error.message : 'Terjadi kesalahan saat memuat data purchase orders'}
              </p>
