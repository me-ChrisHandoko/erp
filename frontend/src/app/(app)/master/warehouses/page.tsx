 ...
 *
 * Server-side rendered page that fetches initial warehouse data
...
import type { WarehouseListResponse } from "@/types/warehouse.types";
...
export default async function WarehousesPage() {
  ...
  // Fetch initial warehouses data on server
  let initialData: WarehouseListResponse;

  try {
    const response = await apiFetch<ApiSuccessResponse<WarehouseListResponse>>({
      endpoint: '/warehouses',
      params: {
        page: 1,
        page_size: 20,
    ...
    console.error('[Warehouses Page] Failed to fetch initial data:', error);
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-1 flex-col items-center justify-center min-h-[400px] gap-4 p-4">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {error instanceof Error ? error.message : 'Terjadi kesalahan saat memuat data gudang'}
              </p>
  ...
  return (
    <ErrorBoundary>
      <div className="flex flex-col">
        ...
        <WarehousesClient initialData={initialData} />
      </div>
    </ErrorBoundary>
  );
}
