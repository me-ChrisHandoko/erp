/**
 * Sales Payments Page - Server Component
 *
 * Server-side rendered page that fetches initial payment data
...
import { SalesPaymentsClient } from "./payments-client";
import { apiFetch } from "@/lib/server/api-fetch";
import { requireAuth } from "@/lib/server/auth";
import type { SalesPaymentListResponse } from "@/types/sales-payment.types";
...
export default async function SalesPaymentsPage() {
  ...
  // Fetch initial payments data on server
  let initialData: SalesPaymentListResponse;

  try {
    const response = await apiFetch<ApiSuccessResponse<SalesPaymentListResponse>>({
      endpoint: '/payments',
      params: {
        page: 1,
        page_size: 20,
    ...
    console.error('[Sales Payments Page] Failed to fetch initial data:', error);
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-1 flex-col items-center justify-center min-h-[400px] gap-4 p-4">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {error instanceof Error ? error.message : 'Terjadi kesalahan saat memuat data pembayaran'}
              </p>
  ...
  return (
    <ErrorBoundary>
      <div className="flex flex-col">
        ...
        <SalesPaymentsClient initialData={initialData} />
      </div>
    </ErrorBoundary>
  );
}
