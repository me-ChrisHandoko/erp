/**
 * Deliveries Page - Server Component
 *
 * Server-side rendered page that fetches initial delivery data
...
import type { DeliveryListResponse } from "@/types/delivery.types";
...
export default async function DeliveriesPage() {
  ...
  // Fetch initial deliveries data on server
  let initialData: DeliveryListResponse;

  try {
    const response = await apiFetch<ApiSuccessResponse<DeliveryListResponse>>({
      endpoint: '/deliveries',
      params: {
        page: 1,
        page_size: 20,
    ...
    console.error('[Deliveries Page] Failed to fetch initial data:', error);
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-1 flex-col items-center justify-center min-h-[400px] gap-4 p-4">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {error instanceof Error ? error.message : 'Terjadi kesalahan saat memuat data pengiriman'}
              </p>
  ...
  return (
    <ErrorBoundary>
      <div className="flex flex-col">
        ...
        <DeliveriesClient initialData={initialData} />
      </div>
    </ErrorBoundary>
  );
}
